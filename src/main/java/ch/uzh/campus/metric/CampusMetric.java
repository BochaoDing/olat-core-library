...
import ch.uzh.campus.connectors.CampusProcessJob;
import ch.uzh.campus.connectors.CampusProcessStep;
...
@Component
@ManagedResource(
        objectName = "ch.uzh.campus.metric:name=campusMetric",
        description = "Campus-Import Service Metric Bean",
        log = true,
        logFile = "jmx.log",
        currencyTimeLimit = 15,
        persistPolicy = "OnUpdate",
        persistPeriod = 200,
        persistLocation = "campusMetric",
...
//        }
    ...
    private String formatStepExecution(StepExecution se) {
        return String.format(
                "status=%s, startTime=%s, endTime=%s, readSkipCount=%s, writeSkipCount=%s, processSkipCount=%s",
                se.getStatus(), se.getStartTime(), se.getEndTime(), se.getReadSkipCount(), se.getWriteSkipCount(), se.getProcessSkipCount()
        );
    }
