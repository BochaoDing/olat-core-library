...
@NamedQueries({
        ...
        @NamedQuery(name = Course.GET_CREATED_COURSES_BY_LECTURER_ID, query = "select c from Course c join c.lecturerCourses lc where lc.lecturer.personalNr = :lecturerId and c.resourceableId is not null and c.enabled = '1' and c.shortSemester= (select max(c2.shortSemester) from Course c2) and c.title like :searchString"),
